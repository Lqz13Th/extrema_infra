...
// use super::{
//     config_assets::{
//         HYPERLIQUID_BASE_URL, HYPERLIQUID_EXCHANGE, HYPERLIQUID_INFO, HYPERLIQUID_MAINNET_SOURCE,
//     },
...
//     let owner_private_key = std::env::var("HYPERLIQUID_OWNER_PRIVATE_KEY").ok();
//     let target_address = std::env::var("HYPERLIQUID_TARGET_ADDRESS").ok();
//
//     Ok(HyperliquidAuth {
//         owner_address,
//         agent_private_key,
...
//     pub async fn post_info<T, B>(&self, client: &Client, body: &B) -> InfraResult<T>
//     where
...
//             .map_err(|e| InfraError::ApiCliError(format!("Serialize info body failed: {}", e)))?;
//
...
//             InfraError::ApiCliError(format!("Serialize exchange body failed: {}", e))
//         })?;
...
//             .map_err(|e| InfraError::ApiCliError(format!("Serialize action failed: {}", e)))?;
